...
for size in [(7, 8)]:
	...
	path_protocol = os.path.join(directory_code_qasmf, "{}.qasmf".format(protocol))
	...
	table_data_qubit = {k: v for k, v in table_qubit_arrangement.items() if "data" in k}
	...
	######################################################
	direction = "vertical"
	extended_layout_size_vertical = {"height": layout_size["height"]*2, "width": layout_size["width"]}

	qchip2_layout = layout_generator.generate_regular_qchip_architecture(job_dir, extended_layout_size_vertical)
	path_qchip_extended_vertical = qchip2_layout["result_file"]

	inverse_qubit_table_LQ1 = {v: "-".join(["LQ1", k]) for k, v in table_qubit_arrangement.items()}
	inverse_qubit_table_LQ2 = {v: "-".join(["LQ2", k.replace("data", "ancilla")]) for k, v in table_qubit_arrangement.items()}
	...
	extended_layout = util.merge_qubit_layout(inverse_qubit_table_LQ1, inverse_qubit_table_LQ2, 
												direction=direction, layout_size=layout_size)
	...
	ic(synthesis_result[protocol])
	util.display_qubit_mapping(synthesis_result[protocol]["system_code"]["final_mapping"], extended_layout_size_vertical)

	table_qubit_arrangement_after_checkup1 = synthesis_result[protocol]["system_code"]["final_mapping"]

	###################################################### 		
	# generate an horizontally extended qubit layout
	######################################################
	direction = "horizon"
	extended_layout_size_full = {"height": layout_size["height"]*2, "width": layout_size["width"]*2}
	qchip2_layout = layout_generator.generate_regular_qchip_architecture(job_dir, extended_layout_size_full)
	path_qchip_extended_full = qchip2_layout["result_file"]

	inverse_qubit_table_LQ1 = {v: k for k, v in table_qubit_arrangement_after_checkup1.items() if "LQ1" in k}
	inverse_qubit_table_LQ2 = {v: k.replace("LQ1", "LQ2").replace("data", "ancilla") for v, k in inverse_qubit_table_LQ1.items()}

	# merge two qubit layouts and allocate the qubits on that
	extended_layout = util.merge_qubit_layout(inverse_qubit_table_LQ1, inverse_qubit_table_LQ2, 
												direction=direction, layout_size=extended_layout_size_vertical)
	...
	ic(synthesis_result[protocol])
	util.display_qubit_mapping(synthesis_result[protocol]["system_code"]["final_mapping"], extended_layout_size_full)
	...
	inverse_qubit_table_LQ2 = {v: k.replace("data", "syndrome") for k, v in table_data_qubit.items()}
...
file_performance = "protocol_performance-{}-{}.json".format(code, job_id)
